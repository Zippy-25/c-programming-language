...
void main(int argc, char *argv[])
{
	...
	while(--argc > 0){	/*(type = getop(s)) != EOF : previous while used getchar and thus, we need to end the loop when EOF occurred. */
				/*but here since we use command line argument, EOF handling is not req.*/
		ungets(" ");
		ungets(*++argv);	
...
void ungets(char s[])
{
	int i;
	i = strlen(s);
	printf("Printing the last char: %c\n", s[i-1]);
	while(i--)	ungetch(s[i]);
}
