/*Exercise 4-8: Our getch and ungetch do not handle a pushed-back EOF correctly. Decide
what their properties ought to be if an EOF is pushed back, then implement your design.*/

/*if we remove c != EOF checking , then EOF will be sent to buf and EOF is -1. right. so -1 is typecasted to char (which is also -1) and then stored).

but real problem is when we give input as 255(number), then then push it back to buf, then 255 will also be considered as EOF..... as here char is considered signed,  this doesn't happen.
but it is a limitation of getch() and ungetch().

so to remove this, make the buf int*/
